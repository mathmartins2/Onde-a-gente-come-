...
check "sessao sorteada fica fechada" "$(query "SELECT count(*) FROM draw_sessions WHERE draw_id IS NOT NULL AND status NOT IN ('drawn','revealing');")" "0"
...
check "toda nota tem os 6 criterios" "$(query "SELECT count(*) FROM ratings WHERE flavor_score IS NULL OR price_score IS NULL OR service_score IS NULL OR ambience_score IS NULL OR menu_score IS NULL OR wait_time_score IS NULL;")" "0"
check "nota final bate com a media dos criterios" "$(query "SELECT count(*) FROM ratings WHERE abs(score - (flavor_score + price_score + service_score + ambience_score + menu_score + wait_time_score) / 6.0) > 0.01;")" "0"
